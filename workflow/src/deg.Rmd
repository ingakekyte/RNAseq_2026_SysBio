...
# Make sample IDs row names and match count matrix order
samplesheet <- as.data.frame(samplesheet)
rownames(samplesheet) <- samplesheet$ID
samplesheet <- samplesheet[colnames(count_matrix), , drop = FALSE]

head(samplesheet)
table(samplesheet$phenotype, samplesheet$sex)
...
p <- plotPCA(vsd, intgroup = c("phenotype", "sex")) + 
      theme_minimal() +
...
n_top <- min(500, nrow(assay(vsd)))
top <- names(sort(vars, decreasing = TRUE))[1:n_top]
mat <- assay(vsd)[top, ]


# create annotation data frame for samples
ann_col <- as.data.frame(colData(vsd)[, c("phenotype", "sex"), drop = FALSE])
...
rownames(sampleDistMatrix) <- paste(vsd$ID, vsd$phenotype, vsd$sex, sep="-")
...
#res <- results(dds_filt, contrast = c("phenotype", "Maternal_0.3Cyp", "Ctrl"))
#res <- results(dds_filt, contrast = c("phenotype", "Paternal_0.3Cyp", "Ctrl"))
res <- results(dds_filt, contrast = c("phenotype", "Both_0.3Cyp", "Ctrl"))
res
summary(res)
```
```{r}

#resLFC <- lfcShrink(dds_filt, coef="phenotype_Maternal_0.3Cyp_vs_Ctrl", type="apeglm")
#resLFC <- lfcShrink(dds_filt, coef="phenotype_Paternal_0.3Cyp_vs_Ctrl", type="apeglm")
resLFC <- lfcShrink(dds_filt, coef="phenotype_Both_0.3Cyp_vs_Ctrl", type="apeglm")
resLFC
...
top_genes$symbol <- symbols[top_genes$ENSEMBL]
